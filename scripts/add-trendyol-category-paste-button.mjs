...
if (!src.includes('data-testid="button-paste-trendyol-category-url"')) {
  const inputRe = /\s{18}<Input\n\s+value=\{url\}\n\s+onChange=\{\(event\) => setUrl\(event\.target\.value\)\}\n\s+placeholder="[^"]+"\n\s+className="business-input h-11 border-zinc-800 bg-zinc-900\/70 text-sm"\n\s+disabled=\{loading\}\n\s+data-testid="input-trendyol-category-url"\n\s+\/>/;
  const match = src.match(inputRe)?.[0];
  if (!match) throw new Error("[category-paste] URL input anchor not found");
  const placeholder = match.match(/placeholder="([^"]+)"/)?.[1] || "https://www.trendyol.com/elektrikli-ev-aletleri-x-c1104";
