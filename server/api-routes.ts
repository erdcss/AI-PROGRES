...
import { universalScrape } from './universal-scraper';
...
router.post('/api/scrape', async (req, res) => {
  try {
    ...
    // Direct extraction without broken dependencies
    console.log('🎯 Direct extraction for comprehensive data');
    ...
    try {
      ...
      let price = 250; // Default for Dyson products
      const pricePatterns = [
      ...
      // Brand-specific features for Dyson
      ...
      console.log(`✅ Direct extraction successful: ${title}, ${features.length} features, ${images.length} images`);
