...
// Shopify upload — merkezi token resolver + standart upload servisi
router.post('/api/shopify-upload', async (req, res) => {
  const { handleShopifyProductsRoute } = await import('./shopify-route-handler');
  return handleShopifyProductsRoute(req, res);
});

// Shopify API endpoint — standart upload handler
router.post('/api/shopify/add-product', async (req, res) => {
  const { handleShopifyProductsRoute } = await import('./shopify-route-handler');
  return handleShopifyProductsRoute(req, res);
});

// Gerçek ürün Shopify'a ekleme — standart upload handler
