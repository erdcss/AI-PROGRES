...
export function ensureMultiUserSchema(): Promise<boolean> {
  if (schemaPromise) return schemaPromise;
  schemaPromise = (async () => {
    if (!pool) {
      console.warn("[multi-user] DATABASE_URL yok; çok kullanıcılı şema başlatılamadı");
      return false;
    }
    try {
      await pool.query(MULTI_USER_SCHEMA_SQL);
      await pool.query("DELETE FROM app_sessions WHERE expires_at <= NOW()");
      console.log("[multi-user] production şeması hazır");
      return true;
    } catch (error) {
      schemaPromise = null;
      console.error("[multi-user] şema hazırlama hatası", error);
      throw error;
    }
  })();
