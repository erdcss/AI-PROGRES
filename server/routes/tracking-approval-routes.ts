...
export function registerTrackingApprovalRoutes(app: Express): void {
  ...
  app.post("/api/tracking/changes/:id/marktgo-sync", async (req, res) => {
    try {
      const id = parsePositiveInt(req.params.id);
      if (id === null) return res.status(400).json({ success: false, error: "Geçersiz ID" });

      const result = await marktGoSyncChange(id, req.body?.actor ?? "user");
      return res.json({ success: true, ...result });
    } catch (err) {
      const msg = (err as Error).message;
      const code = msg.includes("zaten uygulanmış") ? 409 : 422;
  ...
  app.post("/api/tracking/bulk/marktgo-sync", bulkHandler("marktgo-sync"));
...
function bulkHandler(action: "approve" | "reject" | "ignore" | "apply" | "marktgo-sync") {
