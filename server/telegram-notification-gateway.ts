...
export class TelegramNotificationGateway {
  ...
  async sendNotification(
    ...
    if (!this.ALLOWED_TYPES.includes(type)) {
      console.log(
        `⚠️ Unknown notification type: ${type} - blocking for safety`,
      );
      return false;
    }
    ...
    if (metadata && (metadata.color || metadata.size)) {
      ...
      if (this.isFakeVariant(color, size)) {
        console.log(
          `🚫 ULTIMATE BLOCK: Fake variant notification blocked BEFORE database - ${color} / ${size} (${type})`,
        );
        return false;
      }
    }
    ...
    if (type.includes("variant")) {
      console.log(
        `🔍 DEDUP DEBUG: type=${type}, productId=${productId}, variantId=${variantId}, color=${metadata?.color}, size=${metadata?.size}, hash=${hash.substring(0, 16)}...`,
      );
    }
    ...
    if (this.isDuplicate(hash)) {
      console.log(
        `🚫 Duplicate notification blocked: ${type} (within 5 min window)`,
      );
      return false;
    }
    ...
    if (
      productId &&
      !currentBatch &&
      !isVariantNotification &&
      this.isProductThrottled(productId, type)
    ) {
    ...
    if (!(await this.checkTelegramRateLimit())) {
      console.log(
        `🚨 Telegram API rate limit reached - message queued or dropped`,
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        `💾 Notification saved to database (ID: ${notificationId}) with status: pending`,
      );
    } catch (dbError) {
    ...
    try {
      ...
      if (notificationId) {
        try {
          ...
          console.log(
            `✅ Notification sent & database updated (ID: ${notificationId}): ${type} ${productId ? `(Product ${productId})` : ""}`,
          );
        } catch (updateError) {
          console.error(
            `⚠️ Failed to update notification status in database:`,
            updateError,
          );
        }
      } else {
        console.log(
          `✅ Notification sent: ${type} ${productId ? `(Product ${productId})` : ""}`,
        );
      }
      ...
      if (notificationId) {
        try {
          ...
          console.log(
            `❌ Notification failed & database updated (ID: ${notificationId})`,
          );
        } catch (updateError) {
          console.error(
            `⚠️ Failed to update notification failure in database:`,
            updateError,
          );
        }
      }
  ...
  private isFakeVariant(color: string, size: string): boolean {
    ...
    const isFakeSize = FAKE_SIZES.some(
      (fake) => normalizedSize.toLowerCase() === fake.toLowerCase(),
    );
    const isFakeColor = FAKE_COLORS.some(
      (fake) => normalizedColor.toLowerCase() === fake.toLowerCase(),
    );
  ...
  async sendVariantChange(
    ...
    if (this.isFakeVariant(color, size)) {
      console.log(
        `🚫 GATEWAY BLOCK: Fake variant notification blocked - ${color} / ${size} (${changeType})`,
      );
      return false;
    }
    ...
    const colorSafe = String(color || "").trim() || "—";
    const sizeSafe = String(size || "").trim() || "—";
    const variantLabel = [colorSafe, sizeSafe].filter((v) => v !== "—").join(" / ");
    const message =
      `${emoji} ${bgEmoji} <b>${title}</b>\n` +
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      `📦 <b>Ürün:</b> ${productTitle}\n\n` +
      (variantLabel ? `🏷️ <b>${variantLabel} → ${title}</b>\n\n` : '') +
      `🎨 <b>Renk:</b> <code>${colorSafe}</code>\n` +
      `📏 <b>Beden:</b> <code>${sizeSafe}</code>\n\n` +
  ...
  private buildDedupSignature(params: {
    ...
    // 📦 STOCK NOTIFICATIONS: Include stock status and variant info
      ...
      if (
        metadata?.oldStock !== undefined &&
        metadata?.newStock !== undefined
      ) {
  ...
  private async checkTelegramRateLimit(): Promise<boolean> {
    ...
    if (this.messageTimestamps.length >= this.TELEGRAM_RATE_LIMIT) {
      ...
      console.log(
        `⏰ Telegram rate limit: ${this.messageTimestamps.length}/${this.TELEGRAM_RATE_LIMIT} messages sent`,
      );
      console.log(
        `⏰ Need to wait ${Math.ceil(waitTime / 1000)}s before sending next message`,
      );
  ...
  private addToProductBatch(
    ...
    console.log(
      `🎯 Added to batch: Product ${productId} - ${type} (${batch.changes.length} changes)`,
    );
  }
  ...
  private async processBatches(): Promise<void> {
    ...
    for (const [productId, batch] of this.productBatches.entries()) {
      ...
      // If batch has <3 changes, clean up old batches to free memory
        ...
        console.log(
          `🧹 Cleaning up old batch with <3 changes: Product ${productId}`,
        );
        this.productBatches.delete(productId);
      }
    }
   ...
   */
  private async sendBatchedNotificationWithCleanup(
    batch: ProductChangeBatch,
  ): Promise<boolean> {
    // Check rate limit before sending batch
    if (!(await this.checkTelegramRateLimit())) {
      console.log(
        `🚨 Telegram rate limit - batch delayed: Product ${batch.productId}`,
      );
  ...
  private async sendBatchedNotification(
    ...
    const duration = Math.ceil(
      (batch.lastChange - batch.firstChange) / 1000 / 60,
    );
    ...
    if (priceChanges > 0)
      changesSummary += `💰 ${priceChanges} fiyat değişikliği\n`;
    if (variantChanges > 0)
      changesSummary += `🎨 ${variantChanges} varyant değişikliği\n`;
    if (stockChanges > 0)
      changesSummary += `📦 ${stockChanges} stok değişikliği\n`;
    ...
    try {
      ...
      try {
        ...
        console.log(
          `💾 Updated batched notifications to "sent" status for product ${batch.productId}`,
        );
      } catch (dbError) {
        console.error(
          `❌ Failed to update batched notification status:`,
          dbError,
        );
      }

      console.log(
        `🎯 Batched notification sent: Product ${batch.productId} - ${changeCount} changes`,
      );
    } catch (error) {
