...
export const scrapeGatewaySettings = pgTable('scrape_gateway_settings', {
  ...
  lastWorkingProvider: text('last_working_provider'),
  lastTestUrl: text('last_test_url'),
  lastTestHtmlSize: integer('last_test_html_size'),
  lastTestTitleFound: boolean('last_test_title_found'),
  lastTestPriceFound: boolean('last_test_price_found'),
  lastTestImagesFound: integer('last_test_images_found'),
  localAgentEndpoint: text('local_agent_endpoint'),
  localAgentTokenEncrypted: text('local_agent_token_encrypted'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});
