...
- **Unique Tracking ID via Shopify Metafields** (August 13, 2025): Implemented unique product tracking IDs using Shopify custom metafields (custom.replit_id). System generates unique IDs for each product and includes them in CSV exports as "Metafield: custom.replit_id [single_line_text_field]" column. IDs are sent via API when creating products, enabling tracking through Shopify admin panel. Updated csv-generator.ts, multi-variant-csv-generator.ts, and shopify-integration.ts to support metafield handling. Key changed from repli_t_id to replit_id to match Shopify admin field naming. Added separate metafield API call after product creation for better reliability. Updated API version to 2024-01 for improved metafield support.
