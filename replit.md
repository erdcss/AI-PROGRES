...
-   **AI-Powered Product Enhancement:** Integrates OpenAI GPT-4o (via `OPENAI_API_KEY_NEW`) and Google Gemini 2.0 Flash (via `GOOGLE_API_KEY`) for intelligent product analysis, SEO optimization, quality scoring, content generation (descriptions, category matching), target audience analysis, and competitive advantage identification. Both AIs run in parallel for dual-validation: OpenAI and Gemini are called simultaneously, their results are merged, and confidence is boosted when both agree. The `/api/ai-status` endpoint reports live status of all AI services. Anthropic Claude (via `ANTHROPIC_API_KEY`) is used for deep product analysis. Key files: `server/gemini-enhancer.ts` (new), `server/openai-enhanced-extractor.ts`, `server/openai-price-enhancer.ts`.
...
-   **Smart Color Extraction:** Intelligently extracts color names from multiple sources: DOM elements, JavaScript state, and URL slugs. Falls back to URL parsing when DOM extraction fails, preventing "Standart" placeholder colors. Supports compound Turkish color names like "Bebe Mavisi", "Açık Mavi", etc. Priority chain: (1) puppeteer-current-color meta, (2) puppeteer DOM colors, (3) title colors, (4) script/active/URL colors, (5) JSON-LD single color (authoritative for main product), (6) JSON-LD fallback in fake-color replacement block, (7) DsmColor from HTML, (8) frequency-based vote. Fixes: JSON-LD "color" field now used as authoritative fallback when static HTML vote-based system picks wrong color from related products' data (e.g. "Krem" from navigation categories). The `/api/scenario-scrape` endpoint exclusively uses the Puppeteer-based scenario scraper (never the Direct Trendyol API), because the public API returns allVariants for the entire product group (all colors), not the URL-specific color — causing systematic wrong-color extraction. Scenario scraper uses JS state to get the correct URL-specific color and all available sizes with real stock status.
-   **Multi-Color Product Extraction (PREBUILT approach):** For products with multiple color URLs (e.g. 5-color pasyone sneaker p-831888656), the scraper uses a two-step approach without extra HTTP requests: (1) In the speed mode block, HTML regex finds all seller color URLs and JSON-LD `ProductGroup.hasVariant` provides all sizes — the full color×size matrix is built immediately as `prebuiltMultiColorVariants` before any fallback can overwrite data; (2) After variant extraction (which may fall back to Google Cache returning only 1 wrong color), the prebuilt matrix is applied if it has more colors, replacing the fallback result. This gives correct results (e.g. 7 colors × 9 sizes = 63 variants) without server-crashing parallel Puppeteer instances.
...
-   **CSV Generation:** Produces Shopify-compatible CSV files using the official Shopify 2024+ template format (57 columns including: Title, URL handle, Description, Vendor, Product category, Option names/values/Linked To for color swatches, Price, Compare-at price, Inventory settings, Product image URL, SEO fields, Color metafield, Google Shopping columns). Supports all variant combinations (color+size, color-only, size-only, single). Main generator: `server/multi-variant-csv-generator.ts`.
